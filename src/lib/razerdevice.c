...
/**
 * STEP 1 FIX:
 * - Current enumeration uses legacy IOUSBDevice stack (kIOUSBDeviceClassName) and IOUSBDeviceInterface.
 * - Newer devices (like your Pro Click V2 Vertical) show up as IOUSBHostDevice and will not be matched.
 * - Add a fallback scan for IOUSBHostDevice to at least detect them (productId/vendorId from registry props).
 *   This returns devices with usbDevice = NULL (detection-only).
 */

static int get_int_prop(io_service_t svc, const char *key, int *out)
{
    if (!svc || !key || !out) return 0;

    CFStringRef keyStr = CFStringCreateWithCString(kCFAllocatorDefault, key, kCFStringEncodingUTF8);
    if (!keyStr) return 0;

    CFTypeRef ref = IORegistryEntryCreateCFProperty(svc, keyStr, kCFAllocatorDefault, 0);
    CFRelease(keyStr);
    ...
    if (CFGetTypeID(ref) == CFNumberGetTypeID()) {
        ok = CFNumberGetValue((CFNumberRef)ref, kCFNumberIntType, out);
    }
    CFRelease(ref);
...
static RazerDevices getAllRazerHostDevices(void)
{
    RazerDevices allDevices = (RazerDevices){ .devices = NULL, .size = 0 };
    ...
    kern_return_t kReturn = IOServiceGetMatchingServices(MACH_PORT_NULL, matchingDict, &iter);
    ...
    while ((svc = IOIteratorNext(iter))) {
        ...
        int okVid = get_int_prop(svc, "idVendor", &vid);
        int okPid = get_int_prop(svc, "idProduct", &pid);

        IOObjectRelease(svc);

        if (!okVid || !okPid) continue;
        if (vid != USB_VENDOR_ID_RAZER) continue; // Only Razer

        array_size++;
        razerDevices = (RazerDevice *)realloc(razerDevices, array_size * sizeof(RazerDevice));
        array_index++;

        // Detection-only: usbDevice is NULL because legacy IOUSBDeviceInterface won't attach to IOUSBHostDevice
...
RazerDevices getAllRazerDevices()
{
    ...
    int array_size = 0;
    int array_index = -1;
    RazerDevice *razerDevices = (RazerDevice *)malloc(array_size * sizeof(RazerDevice));
    ...
    // STEP 1 fallback: if legacy scan found nothing, try IOUSBHostDevice detection
    if (array_size == 0)
    {
        free(razerDevices);
...
void closeAllRazerDevices(RazerDevices devices)
{
    for (int counter = 0; counter < devices.size; ++counter)
    {
        ...
        // Detection-only devices (IOUSBHostDevice fallback) have usbDevice = NULL
